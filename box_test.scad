...
translate([wall_th,wall_th,0]) {
    ...
    // Top end panel
    if (show_top_end) let(wall_th=wall_th-.1) color("blue",0.75) translate([0, 0, z-wall_th]) lasercutoutSquare(
        thickness=wall_th, x=x-wall_th*2, y=y-wall_th*2,
    ...
    // Right side panel
    if (show_right_side) let(wall_th=wall_th-.1) color("green", 0.75) translate([0,y-wall_th,wall_th]) rotate([90,0,0]) lasercutoutSquare(
        thickness=wall_th, x=x-wall_th*2, y=z-wall_th*2,
        finger_joints = fj([1,1,1,0]),
        circles_remove=[
            [ 1.1, 4, 4 ],
    ...
    // Top side panel
    if (show_top_side) let(wall_th=wall_th+.1)  color("purple", 0.75) translate([0,0,wall_th]) rotate([0,-90,0]) lasercutoutSquare(
    ...
    // Bottom side panel
    if (show_bottom_side) let(wall_th=wall_th+.1) color("brown", 0.75) translate([x-wall_th,0,wall_th]) rotate([0,-90,0]) lasercutoutSquare(
        thickness=wall_th, x=z-wall_th*2, y=y-wall_th*2,
        finger_joints = fj([0,1,0,1]),
        circles_remove=[
            [1, 3, 3],
